...
figure(1); ax(2) = subplot(2,1,1); plot(path.s_m,path.ax_des); title('ax_des');
figure(1);ax(1) = subplot(2,1,2); plot(path.s_m,vx_prof);  title('vx_des');
linkaxes(ax,'x');
% path for project
%load('project_data.mat');
...
%% CALL TO ME227 CONTROLLER
    [delta_steer, Fx_total] = me227_controller(s, e, delta_psi, ux, uy, r, 1, path);
