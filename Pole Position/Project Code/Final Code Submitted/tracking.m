...
% vx_prof(finish) = sqrt(vx_prof(finish-1)^2 + 2*ax_prof(finish)*ds);
% 
%% 17 Final Straight
v_straight_final = vx_prof(s_xcl8_idx);
s_desired_finish = path.s_m(finish) - path.s_m(s_xcl8_idx)-3;
...
for i = s_xcl8_idx:finish - 1
    ax_prof(i+1) = braking_accel;
    if (  (vx_prof(i)^2 + 2*ax_prof(i+1)*ds) < 0) %we are about to command im.vx_prof
       vx_prof(i+1) = -0.1;
       ax_prof(i+1) = 0; 
    else
       vx_prof(i+1) = sqrt(vx_prof(i)^2 + 2*ax_prof(i+1)*ds);
...
% 
% for i = idx_stopped:finish-1
%     vx_prof(i+1) = -0.1;
%     ax_prof(i+1) = 0;
% end
% vx_prof(finish) = vx_prof(finish-1);
% ax_prof(finish) = braking_accel;
